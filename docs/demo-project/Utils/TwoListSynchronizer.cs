...
namespace Ofcas.Lk.Api.Client.Demo.Utils
{
    ...
    /// </summary>
    internal class TwoListSynchronizer : IWeakEventListener
    {
        ...
        public TwoListSynchronizer(IList masterList, IList targetList, IList itemsSource)
            : this(masterList, targetList, DefaultConverter)
        {
            _itemsSource = itemsSource;
        }

        private delegate void ChangeListAction(IList list, IList itemsSource, NotifyCollectionChangedEventArgs e, Converter<object, object> converter);
        ...
        private static void AddItems(IList list, IList itemsSource, NotifyCollectionChangedEventArgs e, Converter<object, object> converter)
        {
            ...
            if (itemsSource != null)
            {
                var castedList = list.OfType<object>();
                var sortedList = castedList.OrderBy(x => itemsSource.IndexOf(x)).ToList();
                list.Clear();
        ...
        private static void MoveItems(IList list, IList itemsSource, NotifyCollectionChangedEventArgs e, Converter<object, object> converter)
        {
            RemoveItems(list, itemsSource, e, converter);
            AddItems(list, itemsSource, e, converter);
        }

        private static void RemoveItems(IList list, IList itemsSource, NotifyCollectionChangedEventArgs e, Converter<object, object> converter)
        {
            var itemCount = e.OldItems.Count;

            for (var i = 0; i < itemCount; i++)
                list.Remove(e.OldItems[i]);
        }

        private static void ReplaceItems(IList list, IList itemsSource, NotifyCollectionChangedEventArgs e, Converter<object, object> converter)
        {
            RemoveItems(list, itemsSource, e, converter);
            AddItems(list, itemsSource, e, converter);
        }
        ...
        private void PerformActionOnList(IList list, ChangeListAction action, NotifyCollectionChangedEventArgs collectionChangedArgs, Converter<object, object> converter)
        {
            ...
            action(list, _itemsSource, collectionChangedArgs, converter);
